...
$thenotification="$stock_num number of $stockname share have been PUT up for sale at Rs. $price_per_share";
$currenttimestamp=time();
...
$thenotification="$stock_num number of $stockname share have been BID FOR BUYING at Rs. $price_per_share";
$currenttimestamp=time();
...
$thenotification="$stock_num number of $thestockname share have been MORTGAGED for cash amount Rs. $totalpriceofshares at $rateofmortgage per share";
$currenttimestamp=time();
...
$getthegraphpoints=mysql_query("SELECT * FROM `graph` WHERE `stockid`='{$theid}' LIMIT $num_getthegraphpoints_l,$num_getthegraphpoints");
error_log("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
error_log(mysql_error());
...
{
$myvar[$iiuy]=array("X"=>($iiuy),"Y"=>$myvari[$iiuy]);
}
...
while($grph=mysql_fetch_array($getthegraphpoints) )
{
$myvar[$c_2]=array("X"=>($c_2),"Y"=>(int)$grph[2]);
$c_2++;
}
}
...
$thenotification="$stocknum number of $stockname share have been TRADED with EXCHANGE for $totalpriceofstocks at Rs. $currentprice PER SHARE : total CASH in account $finalcashwithuser";
$currenttimestamp=time();
...
$time=time();
$not="$thenum NUMBER of $thename shares have been redeemed : CASH DEBITED -$totalvale_to_be_redeemed :CURRENT CASH :$finalcash";
